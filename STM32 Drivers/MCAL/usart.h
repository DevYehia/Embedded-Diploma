...
typedef struct{
	uint32_t baud_rate;
	uint8_t  sync_mode;     //takes values from USART_SYNC_MODES
	uint8_t  parity_mode;   //takes values from USART_PARITY_MODES
	uint8_t  num_stop_bits; //takes values from USART_STOP_BITS_NUMBER
	uint8_t  char_size;     //takes values from USART_CHAR_LENGTH
	uint8_t  enable_sel;     //takes values from USART_ENABLE_MODES
	uint8_t  enable_irq;     //takes values from USART_INTERRUPT_ENABLE_MODES
} USART_config_t;
